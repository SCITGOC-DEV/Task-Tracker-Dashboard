...
const TwoColumnsDetailsLayout = ({headings, contents, firstColumnTitle, secondColumnTitle, mainTitle = null}) => {
    ...
    return (
        <>
            <div className="w-full bg-white rounded-lg dark:divide-gray-600 divide-gray-200 border">
                {
                    mainTitle && (
                ...
                <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] p-4 gap-6">
                    {/* Left Column */}
                    <div className="p-4">
                        <h2 className="text-xl font-semibold">{firstColumnTitle}</h2>
                        <ul className="mt-2">
                            {leftHeadings.map((heading, index) => (
                                <li key={index} className="flex justify-between border-b py-2">
                                    <span className="font-medium">{heading}:</span>
                                    <span className="text-gray-600">{leftContents[index]}</span>
                                </li>
                            ))}
                        </ul>
                    </div>
